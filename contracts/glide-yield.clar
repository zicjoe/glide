...
(define-private (assert-yield-executor)
  (let
    (
      (executor-response (contract-call? .glide-core is-yield-executor tx-sender))
    )
...
(define-private (assert-strategy-valid (strategy-id uint) (asset uint))
  (begin
    ...
    (match (unwrap-panic (contract-call? .glide-strategy-registry get-strategy strategy-id))
      ...
      none
        (unwrap! none ERR_STRATEGY_NOT_FOUND)
    )
  )
)
  ...
  (match (unwrap-panic (contract-call? .glide-treasury get-bucket merchant-id bucket-id))
    bucket
      (begin
        ...
        (match (unwrap-panic (contract-call? .glide-treasury get-destination merchant-id (get destination-id bucket)))
          destination
            (asserts! (is-eq (get asset destination) asset) ERR_ASSET_MISMATCH)
          none
            (unwrap! none ERR_BUCKET_NOT_FOUND)
        )
        true
      )
    none
