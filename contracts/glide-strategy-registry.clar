...
(define-read-only (is-strategy-active (strategy-id uint))
  (match (map-get? strategies { strategy-id: strategy-id })
    strategy (ok (get active strategy))
    none (ok false)
  )
)
...
(define-read-only (supports-asset (strategy-id uint) (asset uint))
  (match (map-get? strategies { strategy-id: strategy-id })
    strategy (ok (is-eq (get asset strategy) asset))
    none (ok false)
  )
)

(define-private (assert-admin)
  (asserts! (is-eq tx-sender (unwrap-panic (contract-call? .glide-core get-contract-owner))) ERR_UNAUTHORIZED)
)
