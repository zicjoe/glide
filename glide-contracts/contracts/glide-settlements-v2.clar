...
(define-public
  ...
  (begin
    ...
    (let
      (
        (invoice-opt (unwrap-panic (contract-call? .glide-invoices-v2 get-invoice invoice-id)))
      )
      (match invoice-opt
        invoice
          (begin
            ...
            (let
              ...
              (try! (contract-call? .glide-invoices-v2 mark-invoice-paid invoice-id))
              (try! (contract-call? .glide-invoices-v2 link-settlement invoice-id next-id))
              (ok next-id)
            )
          )
